...
class Defender(object):
    ...
    def def_greedy_action_builder(self, G):
        ...
        while not isDup:
            ...
            x = self.nn_def(
                observation=def_input[None],
                stochastic=True,
                mask=mask,
                update_eps=-1,
                training_attacker=False)[0]
    ...
    def def_greedy_action_builder_single(self, G, nn_def):
        self.defact.clear()
        isDup = False
        mask = np.zeros(shape=(1, self.num_nodes+1), dtype=np.float32)
        action_space = self.get_def_actionspace(G)
    ...
    def def_obs_constructor(self, G):
        ...
        def_input = self.prev_obs + self.observation + wasdef + indef
    ...
    def get_def_inDefenseSet(self, G):
        ...
        for node in G.nodes:
            if node in self.defact:
                ...
                indef.append(0)
        return indef
        # indef = np.zeros(G.numNodes)
        # for node in self.defact:
        #     indef[node] = 1
        # return list(indef)
    ...
    def sample_and_set_str(self, str_dict=None):
        assert self.myenv.training_flag == 1  # Should be training the attacker when sampling defender meta-strategy.
        nn, nn_name = ss.sample_strategy_from_mixed(env=self.myenv, str_set=self.str_set, mix_str=self.mix_str, identity=0, str_dict=str_dict)
        self.nn_def = nn
